...
async function init() {
  try {
    // v5.9.131: URL 参数 ?offline=1 设置永久离线标记（存 localStorage，之后所有打开都跳过登录）
    if (typeof location !== "undefined" && location.search && location.search.indexOf("offline=1") >= 0) {
      try { localStorage.setItem("hw_pm_offline_permanent", "1"); } catch (__e0) {}
    }
    // v5.9.131: 永久离线模式（一次性设标记，之后所有打开都进）
    var __offlinePermanent = false;
    try { __offlinePermanent = localStorage.getItem("hw_pm_offline_permanent") === "1"; } catch (__e0b) {}
    if (__offlinePermanent || (typeof location !== "undefined" && location.search && location.search.indexOf("offline=1") >= 0)) {
      var __hideAll2 = function(id) { var el = document.getElementById(id); if (el) el.classList.add("hidden"); };
      __hideAll2("privacy-notice"); __hideAll2("lock-screen"); __hideAll2("auth-screen");
