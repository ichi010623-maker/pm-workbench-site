...
(function (root) {
  ...
  var SYSTEM_PROMPT = [
    ...
    "",
    "【关于痛感强度 pain.intensity】与情绪强度 emotion.intensity 独立：",
    "  - pain.intensity：问题对用户生活的实际影响程度（高=任务中断/扛不住 / 中=困扰/很烦 / 低=轻微/凑合）。",
    "  - emotion.intensity：用户表达情绪的宣泄强度（高=崩溃/绝望 / 中=烦躁/焦虑 / 低=轻微抱怨）。",
    "  同一句话可以有「情绪高 + 痛感低」（宣泄）或「情绪低 + 痛感高」（冷静陈述）。",
    "  不得把情绪强度直接等同于痛感强度。",
    "",
    "【关于持续时间 scene.duration】仅当原文出现时间长度才可判定，否则 unknown：",
    "  brief（几分钟内）/ short（半小时内）/ ongoing（半小时以上）。",
    "",
